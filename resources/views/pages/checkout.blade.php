                                ...
                                <div class="payment">
                                    <div class="card-details">
                                        <form class="checkout-form" method="POST" action="{{ route('orders.store') }}">
                                            @csrf
                                            <button href="{{ route('orders.store') }}" class="btn btn-main mt-20">Place
                                                Order</button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                @foreach ($cartItems as $item)
                                    <div class="media product-card">
                                        <a class="pull-left" href={{ $item->image }}>
                                            <img class="media-object" src='/public/Productimages/{{ $item->image }}'
                                                alt="Image" />
                                        </a>
                                        <div class="media-body">
                                            <h4 class="media-heading"><a href="product-single.html">{{ $item->name }}</a>
                                            </h4>
                                            ...
                                            <form action="{{ route('cart.destroy', $item->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                @csrf
                                                <button class="remove" style="border:none; background:#fff"><span
                                                        class="remove"
